...
public class Function
{
    ...
    {
        { "message", "item deleted from dynamodb" },
    };
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest apigProxyEvent, ILambdaContext context)
    {
        ...
        postId = Uri.UnescapeDataString(postId);

        await DeleteItemAsync(groupId, postId);        
    ...
    public static async Task DeleteItemAsync(string groupId, string postId)
    {
        try {
            var key = new Dictionary<string, AttributeValue>
            {
                ["PK"] = new AttributeValue { S = groupId },
                ["SK"] = new AttributeValue { S = postId },
            };
        ...
        catch (Exception e)
        {
            ...
            {
                { "message", "item deleted from dynamodb failed" },
            };
            statusCode = 500;
