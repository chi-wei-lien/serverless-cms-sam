...
```bash
serverless-cms-dotnet$ sam build
```
...
```bash
serverless-cms-dotnet$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
serverless-cms-dotnet$ sam local start-api
serverless-cms-dotnet$ curl http://localhost:3000/
```
...
```bash
serverless-cms-dotnet$ sam logs -n HelloWorldFunction --stack-name serverless-cms-dotnet --tail
```
...
```bash
serverless-cms-dotnet$ dotnet test test/HelloWorld.Test
```
...
```bash
sam delete --stack-name serverless-cms-dotnet
```
